(function () {
    ...
    describe('Component: Jugend-Panel', function () {
        ...
        it('die Gruppen sollen einen Link auf die entsprechende Gruppen-Seite haben', function () {
            ...
            expect(angular.element(result[0]).find('a').parent().html()).to.contain('data-ui-sref="spi.tgj.gruppe({gruppeid: gruppe.slug || gruppe._id})"');
            expect(angular.element(result[1]).find('a').parent().html()).to.contain('data-ui-sref="spi.tgj.gruppe({gruppeid: gruppe.slug || gruppe._id})"');
